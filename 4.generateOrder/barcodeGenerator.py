...
class Code128:
	...
	def getImage(self, value, height = 50, extension = "PNG"):
	  ...
	  # Create a new image
	  # make the size either the text value or the image value whichever is greater
	  position = 10
	  if len(bits)>font.getsize(value)[0]:
		im = Image.new("L",(len(bits)+position,height))
	  else:
		im = Image.new("L",(font.getsize(value)[0]+position,height))
	  ...
	  # Draw text
	  draw.text((position, height-15), value, font=font, fill=0)
	  
	  # Draw the bar codes
	  for bit in range(len(bits)):
	  # for bit in range(im.size[0]):
		 if bits[bit] == '1':
			draw.rectangle(((bit+position/2,1),(bit+position/2,height-15)),fill=0)
	  im.save("./images/"+value+"."+lower(extension), upper(extension), dpi=(1200,1200), compress_level=9)
...
def generate_barcode(code, size = 45):
    bar = Code128()
    image = bar.getImage(code,size,"png")
    return image
